...
def attrinet_get_parser():
    ...
    parser.add_argument('--guidance_mode', type=str, default='weighted_pseudo_mask',
                        choices=['full_guidance', 'bbox/masks', 'pseudo_mask', 'weighted_pseudo_mask', 'pseudo_bbox', 'mixed', 'mixed_weighted', 'no_guidance', 'guidance_shortcut'])  # use bbox or pseudo_mask as guidance of disease mask for better localization.

    parser.add_argument('--guidance_freq', type=float, default=0.1, help='frequency to train with BBox, default=0.1')
    # Data configuration.
    parser.add_argument('--dataset', type=str, default='nih_chestxray', choices=['chexpert', 'nih_chestxray', 'vindr_cxr', 'vindr_cxr_mix', 'contaminated_chexpert'])
...
def main(exp_configs):
    ...
    if (exp_configs.dataset == "nih_chestxray" or exp_configs.dataset == "vindr_cxr_mix") and  exp_configs.guidance_mode != "no_guidance":
