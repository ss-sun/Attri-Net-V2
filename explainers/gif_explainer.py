...
class gif_explainer():
    ...
    def get_attributions(self, input, target_label_idx, positive_only=False):
        ...
        transform = torchvision.transforms.Compose([xrv.datasets.XRayResizer(320)])
        attr = transform(np.expand_dims(attr, 0))
        return attr
    ...
    def compute_attribution(self, image, model, target_label_idx, ret_params=False, fixrange=None, p=0.0, ae=None,
        ...
        transform = torchvision.transforms.Compose([xrv.datasets.XRayResizer(224)]) # native resolution of the pretrained vae is (224x224)
        image = transform(np.squeeze(to_numpy(image),axis=0))
