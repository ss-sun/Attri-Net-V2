        ...
        if self.with_BBox:
            ...
            for index, row in rows.iterrows():
                ...
                if lesion_type in self.TRAIN_DISEASES:
                    ...
                    label[idx] = 1
                    bb = [int(row['x_min']), int(row['y_min']), int((row['x_max'] - row['x_min'])),
                          int((row['y_max'] - row['y_min']))]
...
class Vindr_CXR_BBOX_MIX_DataModule(LightningDataModule):
    ...
    def __init__(self, dataset_params, split_ratio=0.8, resplit=True, img_size=320, seed=42):
        ...
        self.split_df_dir = os.path.join(self.root_dir, 'Mix_BBox_split_df')
    ...
    def subset(self, src_df, disease, label, transforms):
        ...
        subset = Vindr_CXR_BBOX(image_dir=self.train_image_dir, df=filtered_df, train_diseases=self.TRAIN_DISEASES,
                           transforms=transforms, img_size=self.img_size, with_BBox=False)
        return subset
    ...
    def subset_BBox(self, src_df, disease, transforms):
        ...
        filtered_df = filtered_df.reset_index(drop=True)
        subset = Vindr_CXR_BBOX(image_dir=self.train_image_dir, df=filtered_df, train_diseases=self.TRAIN_DISEASES,
                                transforms=transforms, img_size=self.img_size, with_BBox=True)
        return subset
...
if __name__ == '__main__':
    ...
    vindr_cxr_mix_dict = {
        ...
        "train_csv_file": "/mnt/qb/work/baumgartner/sun22/data/Vindr-CXR/vinbigdata-chest-xray-abnormalities-detection/annotations/pseudo_guidance/remaining_df.csv",
        # previous annotations_train_resized.csv removed the images that are used for pseudo labeling.
        "BBox_csv_file_train": "/mnt/qb/work/baumgartner/sun22/data/Vindr-CXR/vinbigdata-chest-xray-abnormalities-detection/annotations/pseudo_guidance/pseudo_gudance_df.csv",
        # 75 samples for each disease to generated pseudo guidance. in mixed training, only use these bbox for fair comparions.
    ...
    datamodule = Vindr_CXR_BBOX_MIX_DataModule(vindr_cxr_mix_dict,
    ...
    for disease in vindr_cxr_mix_dict["train_diseases"]:
